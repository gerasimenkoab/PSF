...
class DeblurPredictor:
    ...
    def make2dStackPrediction(self, imgToPredict, window):
        ...
        pb = ttk.Progressbar(window, orient='horizontal', mode='determinate', maximum=len(imgLayers), value=0)
        pb.grid(row=10, column=2)

        # deconvolve layers
        resLayers = []
        for layer in imgLayers:
            resLayers.append(self.model.predict(layer.reshape(1, rows, cols, 1)).reshape(rows, cols))
