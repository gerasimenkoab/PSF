...
class ImageChunk:
    def __init__(self, imgSource, rowStart, colStart, layers, chunkSize, offsetSize, rowOffsetBegin, rowOffsetEnd, colOffsetBegin, colOffsetEnd):
        self.chunkData = np.zeros(shape=[layers, chunkSize + 2 * offsetSize, chunkSize + 2 * offsetSize])
        self.chunkData[:, offsetSize - rowOffsetBegin : offsetSize + chunkSize + rowOffsetEnd, offsetSize - colOffsetBegin : offsetSize + chunkSize + colOffsetEnd] = imgSource[:, rowStart - rowOffsetBegin : (rowStart + chunkSize + rowOffsetEnd), colStart - colOffsetBegin : (colStart + chunkSize + colOffsetEnd)]
    ...
    def GetChunkWithoutOffset(self):
        chunkWithoutOffset = self.chunkData[:, self.offset : self.dataRows - self.offset, self.offset : self.dataCols - self.offset]
...
class BigImageManager:
    ...
    def __init__(self, img, chunkSizeBorder = 48, offsetSize = 8):
        ...
        if not isNeedToInflate:
            ...
            self._inflatedImg = np.zeros(shape=[self._imgLayers, self._inflatedRows, self._inflatedCols])
    ...
    def SeparateInChunks(self):
        ...
        while True:
            ...
            while True:
                ...
                chunkRows = min(self._inflatedRows - chunkRowStart, self._chunkSizeBorder)
                chunkCols = min(self._inflatedCols - chunkColStart, self._chunkSizeBorder)

                # offsets init
                chunkRowOffsetBegin = 0 if chunkRowStart - self._offsetSize < 0 else self._offsetSize
                chunkRowOffsetEnd = 0 if chunkRowStart + chunkRows == self._inflatedRows else self._offsetSize
                chunkColOffsetBegin = 0 if chunkColStart - self._offsetSize < 0 else self._offsetSize
                chunkColOffsetEnd = 0 if chunkColStart + chunkCols == self._inflatedCols else self._offsetSize

                # Generate new chunk
                newChunk = ImageChunk(self._inflatedImg, chunkRowStart, chunkColStart, self._imgLayers, self._chunkSizeBorder, self._offsetSize, chunkRowOffsetBegin, chunkRowOffsetEnd, chunkColOffsetBegin, chunkColOffsetEnd)
    ...
    def ConcatenateChunksIntoImage(self, chunksList):
        newImgInflate = np.zeros(shape=[self._imgLayers, self._inflatedRows, self._inflatedCols])

        for chunk in chunksList:
            chunkWithoutOffset = chunk.GetChunkWithoutOffset()
            newImgInflate[:, chunk.rowStart : (chunk.rowStart + chunk.rows), chunk.colStart : (chunk.colStart + chunk.cols)] = chunkWithoutOffset[:, :, :]
