...
class PSFExtractorGUI(Toplevel):
    ...
    def __init__(self, parent):
        ...
        Button(self, text = 'Select Bead Image', command = self.SelectBeadImage).grid(row=2,column=2)
        Button(self, text = 'Load image file', command = self.LoadBeadImageFile).grid(row=2,column=3)

        Button(self, text = 'Launch bead extractor').grid(row=3,column=1)
        Button(self, text = 'Show loaded bead image').grid(row=3,column=2)
        Button(self, text = 'Center image intensity',command = self.CenteringImageInt).grid(row=3,column=3)
        ...
        Button(self, text = 'Calculate PSF', command = self.CalculatePSF).grid(row=7,column=1)
        ...
        Button(self, text = 'Save PSF multi-file',command=self.SavePSFMulti).grid(row=10, column=1)
        Button(self, text = 'Save PSF single-file',command=self.SavePSFSingle).grid(row=10, column=2)

        Button(self, text = 'Load Image',command = self.LoadCompareImageFile).grid(row=11,column=1)
        Button(self, text = 'Load PSF', command = self.LoadPSFImageFile).grid(row=11,column=2)
        Button(self, text = 'Deconvolve',command = self.DeconvolveIt).grid(row=11,column=3)
